...
bool Popup::init() {
    ...
    background->setPosition(Vec2(origin.x + background->getContentSize().width / 5
		,origin.y + background->getContentSize().height / 5));
...
void Popup::appear() {
    ...
    m_popupLayer->runAction(Sequence::create(MoveTo::create(0.15f,Vec2(origin.x, origin.y + background->getContentSize().height / 5)),
                                             callAppear, nullptr));
